...
from molbind.utils.spec2struct import (
    get_atom_counts_from_formula,
    smiles_to_molecular_formula,
)
...
def calculate_mf_penalty(smi: str, atom_counts_orig: dict) -> float:
    ...
    penalty = sum(
        abs(counts_i.get(s, 0) - atom_counts_orig.get(s, 0))
        for s in set(counts_i) | set(atom_counts_orig)
    )
...
def reward_function_ga(
    ...
    for spec, target_emb_1D_gpu in target_1D_embs.items():  # target_emb is 1D (D,)
        if (
            spec not in cand_smiles_embs
            or cand_smiles_embs[spec] is None
            or cand_smiles_embs[spec].nelement() == 0
        ):
    ...
    is_radical_or_charged = np.array(
        [
            smiles_is_radical_or_is_charged_or_has_wrong_valence(smi)
            for smi in individuals
        ]
    )
...
def calculate_detailed_scores(
    smi: str, models: dict, target_1D_embs: dict, atom_counts_orig: dict
) -> dict:
    ...
    for spec, target_emb in target_1D_embs.items():
        ...
        if (
            spec not in smi_emb_dict
            or smi_emb_dict[spec] is None
            or smi_emb_dict[spec].nelement() == 0
        ):
...
def run_ga_instance(
    ...
    ga_res = default_ga(
        ...
        scoring_function=CachedBatchFunction(reward_f, spectra_hash, initial_pop),
        ...
        offspring_gen_func=partial(
            graph_ga_blended_generation, frac_graph_ga_mutate=frac_mutate
        ),
    )

    best_sc, best_smi = (
        max(ga_res.population, key=lambda x: x[0])
        if ga_res.population
        else (-float("inf"), "N/A")
    )
...
def spec2struct(
    ...
    def canonicalize_molecular_formula(atom_count_dict):
        ...
        carbon_hydrogen_mol_formula = (
            f"C{carbon_count}H{hydrogen_count}"
            if carbon_count > 0 or hydrogen_count > 0
            else ""
        )
        ...
        return carbon_hydrogen_mol_formula + "".join(
            sorted(
                ...
                for el, count in atom_count_dict.items()
                if count > 0 and el not in ("C", "H")
            )
        )
    ...
    active_ga_model_modalities = [
        m for m, model in ga_models_for_scoring.items() if model
    ]
    final_ga_models_to_use = {
        m: ga_models_for_scoring[m]
        for m in active_ga_model_modalities
        if m in ga_models_for_scoring
    }
    ...
    analyzer_user_active_spectra = [
        m
        for m in SimpleMoleculeAnalyzer.ALL_SPECTRA_TYPES
        if locals().get(f"analyzer_{m}_exp")
    ]
    ...
    analyzer.load_models(
        ir_experiment=ga_ir_exp,
        cnmr_experiment=ga_cnmr_exp,
        hnmr_experiment=ga_hnmr_exp,
        hsqc_experiment=ga_hsqc_exp,
    )
    ...
    spectrum_as_tensor = (
        torch.tensor(spectrum["y"], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    )
    spectrum_as_tensor.requires_grad = False
    spectrum_as_tensor = spectrum_as_tensor / spectrum_as_tensor.max()
    output_hnmr_embedding = analyzer.models["hnmr"].encode_modality(
        spectrum_as_tensor.detach(), "h_nmr"
    )
    ...
    if initial_environment:
        ...
        response = requests.post(
            ...
            json={
                "reagents": initial_environment["reagents"],
                "reactants": initial_environment["reactants"],
            },
        )
        initial_pop = response.json()["smiles"]
    else:
        torch.nn.functional.normalize(
            target_embeddings["hnmr"], p=2, dim=0, out=target_embeddings["hnmr"]
        )
        ...
        response = requests.post(
            docker_vector_db, json={"mf": mf, "spectrum_embedding": spectrum_embedding}
        )
        initial_pop = response.json()["smiles"]
